...
class BERT(nn.Module):
    ...
    def __init__(self, seq_len, feature_dim, embed_dim=128, depth=4,
        ...
        mask_ratio=0.7, norm_loss=False, use_cls=True, mask_future=False):
        super().__init__()
    ...
    def future_masking(self, x):
        """
        Simple future-only mask.
        x: [N, L, D], sequence
        """
        ...
        ids_restore = torch.arange(start=0, end=L, device=x.device).repeat(N, 1)

        # only keep first indices
    ...
    def forward_encoder(self, x):
        ...
        if self.mask_future:
            x, mask, ids_restore = self.future_masking(x)
        else:
