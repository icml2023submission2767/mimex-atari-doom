...
class DoomEnv(gym.Wrapper):
    def __init__(self, env, is_render, noop_max=30):
        """Sample initial states by taking random number of no-ops on reset.
        No-op is assumed to be action 0.
        """
        gym.Wrapper.__init__(self, env)
        # most recent raw observations (for max pooling across time steps)
        self.observation_space = self.env.observation_space['screen']
        self.noop_max = noop_max
        self.override_num_noops = None
        self.noop_action = 0

        # most recent raw observations (for max pooling across time steps)
        self._obs_buffer = np.zeros((2,) + self.observation_space.shape, dtype=np.uint8)
        self._skip = 4
    ...
    def step(self, action):
        ...
        for i in range(self._skip):
            ...
            if self.is_render:
                self.env.render()
            if i == self._skip - 2:
                self._obs_buffer[0] = obs['screen']
            if i == self._skip - 1:
                self._obs_buffer[1] = obs['screen']
            total_reward += reward
            if done:
                break
...
class DoomEnvironment(Environment):
    def __init__(
        ...
        self.daemon = True
        # self.env = MaxAndSkipEnv(NoopResetEnv(gym.make(env_id)), is_render)
        self.env = DoomEnv(gym.make(env_id), is_render)
        self.env_id = env_id
        self.is_render = is_render
        self.env_idx = env_idx
        self.steps = 0
